...
11. **40.41–40.85** — completed sequentially through §4.1–§4.7 as tabulated above, ending with E2EE Metadata Minimization (`src/modules/e2ee-metadata/`), Message Search Pt.2 (`src/modules/search/` MeiliSearch), Media Pipeline Pt.2 (`src/modules/media/`), Background Jobs (`src/modules/jobs/` BullMQ full set), Structured Logging (`src/core/logger/` pino redaction + `StructuredLogger` helper), Distributed Tracing (`src/core/tracing/` ALS `TraceService` + `TraceMiddleware`), Production Metrics (`src/core/metrics/` `MetricsService` + `GET /metrics`), the 1M-User Load Model (`src/core/load-model/` + `scripts/load/load-baseline.mjs`), Multi-Instance WebSocket Scaling (`src/core/redis/redis-node-registry.ts` + adapter hardening), Load-model-driven DB/query optimisation (`src/core/db/query-optimizer/` + `GET /load-model/optimiser`), the Cache Architecture (`src/core/cache/` + `GET /load-model/cache`), API Versioning / Evolution (`src/core/api-versioning/` + `GET /v1/api` & `GET /v2/api` + `ws-envelope`), and OpenAPI/Swagger Hardening (`src/config/swagger/` `buildSwaggerConfig` + `src/config/validation/` `VALIDATION_PIPE_OPTIONS`), advancing the Scale phase (40.80–40.100).
12. Continue sequentially through **§4.8**; next is **40.86 — (API & WebSocket Contract Tests, next Scale-phase step)**.
...
**Lecture 40.86 — (API & WebSocket Contract Tests, next Scale-phase step).** 40.85 is complete.

**Lecture 40.85 — OpenAPI/Swagger Hardening — COMPLETED**
- **Why:** the API surface is versioned (40.84) and fast-growing; the generated OpenAPI/Swagger documentation and the request-validation contract must be deterministic, complete, and in lock-step with what the API actually accepts — so clients can build typed, accurate consumers and undocumented fields are rejected.
- **How:** refactored `src/config/swagger/swagger.config.ts` into a pure, unit-testable `buildSwaggerConfig(version)` whose `info.version` tracks the current API major version (from the 40.84 registry), plus declared documentation tags (`NEXUS_DOCUMENTATION_TAGS`), a root server entry, an enriched JWT bearer security scheme, and hardened `SwaggerCustomOptions` (persist auth, sorted tags/operations). Refactored `src/config/validation/validation.config.ts` to export `VALIDATION_PIPE_OPTIONS` so the strict `whitelist` / `forbidNonWhitelisted` / `forbidUnknownValues` / implicit-conversion contract is the single source of truth configured on the app and exercised by tests. Cleaned up the dead, unreferenced `src/prisma.controller.ts` (`TestController`, `@Controller('test')`).
- **Integration:** kept 40.77 redaction, 40.78 trace-id, 40.79 `/metrics`, 40.80 load model, 40.81 node registry, 40.82 optimiser, 40.83 cache, and 40.84 versioning/deprecation intact; swagger remains env-gated (`SWAGGER_ENABLED`/NODE_ENV), validation remains strict and non-breaking for documented DTOs. Verified 97 Jest suites / 624 tests green, tsc 0 errors, nest build clean.

**Next lecture after this: 40.86 (API & WebSocket Contract Tests).**
