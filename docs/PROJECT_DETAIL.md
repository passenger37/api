...
11. **40.41–40.84** — completed sequentially through §4.1–§4.7 as tabulated above, ending with E2EE Metadata Minimization (`src/modules/e2ee-metadata/`), Message Search Pt.2 (`src/modules/search/` MeiliSearch), Media Pipeline Pt.2 (`src/modules/media/`), Background Jobs (`src/modules/jobs/` BullMQ full set), Structured Logging (`src/core/logger/` pino redaction + `StructuredLogger` helper), Distributed Tracing (`src/core/tracing/` ALS `TraceService` + `TraceMiddleware`), Production Metrics (`src/core/metrics/` `MetricsService` + `GET /metrics`), the 1M-User Load Model (`src/core/load-model/` + `scripts/load/load-baseline.mjs`), Multi-Instance WebSocket Scaling (`src/core/redis/redis-node-registry.ts` + adapter hardening), Load-model-driven DB/query optimisation (`src/core/db/query-optimizer/` + `GET /load-model/optimiser`), the Cache Architecture (`src/core/cache/` + `GET /load-model/cache`), and API Versioning / Evolution (`src/core/api-versioning/` + `GET /v1/api` & `GET /v2/api` + `ws-envelope`), advancing the Scale phase (40.80–40.100).
12. Continue sequentially through **§4.8**; next is **40.85 — (OpenAPI/Swagger Hardening, next Scale-phase step)**.
...
**Lecture 40.85 — (OpenAPI/Swagger Hardening, next Scale-phase step).** 40.84 is complete.

**Lecture 40.84 — API Versioning / Evolution — COMPLETED**
- **Why:** the backend had a large, fast-growing REST+WS surface; before more read/write endpoints land, it needed an explicit versioning contract and an evolution/deprecation path so future changes never break existing clients.
- **How:** confirmed and hardened the URI-path strategy (`VersioningType.URI` + `defaultVersion: '1'`), so the entire pre-40.84 surface is implicitly `v1` with unchanged paths/behaviour. Added `src/core/api-versioning/` — a data-driven `ApiVersionControlService` + registry (supported/current/oldest, deprecation flags, sunset dates, successors) and a `DeprecationHeaderInterceptor` (app-wide `APP_INTERCEPTOR`) that attaches `Deprecation` / `Sunset` / `Link` headers to any deprecated (`/v1`) response. A versioned public controller serves the same `/api` route at `GET /v1/api` (legacy, deprecated) and `GET /v2/api` (current, evolved view with deprecation/migration metadata) proving two versions of one endpoint coexist. Added a backward-compatible WS event envelope (`src/common/websocket/versioning/ws-envelope.ts`): v1 frames are byte-identical, v2 wraps in `{ v, data }` with a suffixed event name; `parseEnvelope` tolerates both.
- **Integration:** kept 40.77 redaction, 40.78 trace-id, 40.79 `/metrics`, 40.80 load model, 40.81 node registry, 40.82 optimiser, and 40.83 cache intact; `/v1` responses are now deprecated but unchanged, current routes stay non-breaking. Verified 95 Jest suites / 615 tests green, tsc 0 errors, nest build clean.

**Next lecture after this: 40.85 (OpenAPI/Swagger Hardening).**
