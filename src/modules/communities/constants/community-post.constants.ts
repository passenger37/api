...
export const COMMUNITY_POST_VISIBILITY_OPTIONS = ['COMMUNITY_MEMBERS', 'PUBLIC'] as const;

export const COMMUNITY_POST_CONTENT_TYPE_OPTIONS = ['TEXT', 'MEDIA', 'MIXED', 'LINK', 'POLL'] as const;

export const COMMUNITY_POST_STATUS_OPTIONS = ['ACTIVE', 'HIDDEN', 'LOCKED', 'MODERATION_PENDING', 'DELETED'] as const;
...
export const POST_SORT_OPTIONS = ['LATEST', 'TOP', 'HOT', 'CONTROVERSIAL'] as const;

export const COMMUNITY_POST_CURSOR_VERSION = 'v1';

export function encodeCommunityPostCursor(createdAt: Date, id: string, isPinned: boolean): string {
  const payload = `${COMMUNITY_POST_CURSOR_VERSION}|${isPinned ? '1' : '0'}|${createdAt.toISOString()}|${id}`;
  return Buffer.from(payload).toString('base64url');
}

export function decodeCommunityPostCursor(cursor: string): { createdAt: Date; id: string; isPinned: boolean } | null {
  try {
...
export const COMMUNITY_FEED_SORTS_ALL = ['LATEST', 'TOP', 'HOT', 'CONTROVERSIAL'] as const;
export const COMMUNITY_FEED_SORTS_KEYED = ['TOP', 'HOT', 'CONTROVERSIAL'] as const;
