...
export class CommunityPostRules {
  static validateTitle(title: string): { valid: boolean; error?: string } {
    ...
    if (title.length < COMMUNITY_POST_RULES.TITLE.MIN_LENGTH) {
      return {
        valid: false,
        error: `Title must be at least ${COMMUNITY_POST_RULES.TITLE.MIN_LENGTH} characters`,
      };
    }
    if (title.length > COMMUNITY_POST_RULES.TITLE.MAX_LENGTH) {
      return {
        valid: false,
        error: `Title cannot exceed ${COMMUNITY_POST_RULES.TITLE.MAX_LENGTH} characters`,
      };
    }
    return { valid: true };
  }
  ...
  static validateContent(content: string): { valid: boolean; error?: string } {
    ...
    if (content.length < COMMUNITY_POST_RULES.CONTENT.MIN_LENGTH) {
      return {
        valid: false,
        error: `Content must be at least ${COMMUNITY_POST_RULES.CONTENT.MIN_LENGTH} characters`,
      };
    }
    if (content.length > COMMUNITY_POST_RULES.CONTENT.MAX_LENGTH) {
      return {
        valid: false,
        error: `Content cannot exceed ${COMMUNITY_POST_RULES.CONTENT.MAX_LENGTH} characters`,
      };
    }
    return { valid: true };
  }
  ...
  static validateMediaCount(count: number): { valid: boolean; error?: string } {
    if (count > COMMUNITY_POST_RULES.MEDIA_MAX_COUNT) {
      return {
        valid: false,
        error: `Cannot attach more than ${COMMUNITY_POST_RULES.MEDIA_MAX_COUNT} media items`,
      };
    }
    return { valid: true };
  }

  static validateHashtagCount(count: number): {
    valid: boolean;
    error?: string;
  } {
    if (count > COMMUNITY_POST_RULES.HASHTAG_MAX_COUNT) {
      return {
        valid: false,
        error: `Cannot use more than ${COMMUNITY_POST_RULES.HASHTAG_MAX_COUNT} hashtags`,
      };
    }
    return { valid: true };
  }

  static validateMentionCount(count: number): {
    valid: boolean;
    error?: string;
  } {
    if (count > COMMUNITY_POST_RULES.MENTION_MAX_COUNT) {
      return {
        valid: false,
        error: `Cannot mention more than ${COMMUNITY_POST_RULES.MENTION_MAX_COUNT} users`,
      };
    }
    return { valid: true };
  }

  static validateContentWarning(warning: string): {
    valid: boolean;
    error?: string;
  } {
    if (
      warning &&
      warning.length > COMMUNITY_POST_RULES.CONTENT_WARNING_MAX_LENGTH
    ) {
      return {
        valid: false,
        error: `Content warning cannot exceed ${COMMUNITY_POST_RULES.CONTENT_WARNING_MAX_LENGTH} characters`,
      };
    }
    return { valid: true };
  }
  ...
  static validatePostCreation(input: {
    ...
    if (input.contentType) {
      try {
        new (require('../value-objects/post-content-type.vo').PostContentTypeVO)(
    ...
    if (input.visibility) {
      try {
        new (require('../value-objects/post-visibility.vo').PostVisibilityVO)(
          input.visibility as any,
        );
      } catch {
    ...
    if (input.contentWarning) {
      const warningValidation = this.validateContentWarning(
  ...
  static validateReportReason(reason: string): {
    valid: boolean;
    error?: string;
  } {
  ...
  static validateMentionPosition(
    position: number,
  ...
  static extractMentions(
    content: string,
  ): Array<{ username: string; position: number; length: number }> {
  ...
  static canEditPost(
    post: { status: string; isDeleted: boolean; authorUserId: string },
    userId: string,
    isModerator: boolean,
    ...
    if (!status.isEditable()) {
      return {
        allowed: false,
        reason: `Cannot edit a post with status: ${post.status}`,
      };
    }
  ...
  static canDeletePost(
    post: { isDeleted: boolean; authorUserId: string },
    userId: string,
    isModerator: boolean,
  ): { allowed: boolean; reason?: string } {
    if (post.isDeleted) {
  ...
  static canRestorePost(
    post: { isDeleted: boolean },
    isModerator: boolean,
  ): { allowed: boolean; reason?: string } {
    if (!post.isDeleted) {
  ...
  static canVote(
    post: { isDeleted: boolean; status: string },
    userId: string,
    isBanned: boolean,
  ): { allowed: boolean; reason?: string } {
    if (post.isDeleted) {
  ...
  static canReport(
    post: { authorUserId: string; isDeleted: boolean; status: string },
    userId: string,
  ): { allowed: boolean; reason?: string } {
    if (post.isDeleted) {
  ...
  static canBookmark(
    ...
    if (status.isDeleted() || status.isHidden()) {
      return {
        allowed: false,
        reason: 'Cannot bookmark a hidden or deleted post',
      };
    }
  ...
  static getContentTypeFromMedia(
    hasMedia: boolean,
    hasContent: boolean,
  ): CommunityPostContentType {
  ...
  static getPostTypeFromContent(
    contentType: CommunityPostContentType,
  ): 'TEXT' | 'MEDIA' | 'MIXED' | 'LINK' | 'POLL' {
    return contentType;
  }
