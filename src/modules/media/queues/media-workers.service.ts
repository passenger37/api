...
export class MediaWorkersService implements OnModuleInit {
  ...
  private registerThumbnailWorker() {
    this.mediaQueue.registerWorker('media-thumbnail', async (job) => {
      ...
      try {
        ...
        this.logger.error(`Thumbnail generation failed for ${job.data.attachmentId}:`, error);
        throw error;
      }
    }, { concurrency: 2 });
  }
  ...
  private registerTranscodeWorker() {
    this.mediaQueue.registerWorker('media-transcode', async (job) => {
      ...
      try {
        ...
        this.logger.error(`Transcode failed for ${job.data.attachmentId}:`, error);
        throw error;
      }
    }, { concurrency: 1 }); // Limit concurrency for CPU-intensive transcoding
  }
  ...
  private registerAvScanWorker() {
    this.mediaQueue.registerWorker('media-av-scan', async (job) => {
      ...
      try {
        ...
        this.logger.error(`AV scan failed for ${job.data.attachmentId}:`, error);
        throw error;
      }
    }, { concurrency: 4 });
  }
  ...
  private registerMetadataWorker() {
    this.mediaQueue.registerWorker('media-metadata', async (job) => {
      ...
      try {
        ...
        this.logger.error(`Metadata extraction failed for ${job.data.attachmentId}:`, error);
        throw error;
      }
    }, { concurrency: 4 });
  }
  ...
  private async generateThumbnails(attachmentId: string, storageKey: string, mimeType: string) {
    // This is a placeholder - actual implementation would use sharp or similar
    // For now, return mock variants
    return [
      { label: 'small', width: 160, height: 160, key: `thumbnails/${attachmentId}/small.webp`, mimeType: 'image/webp', sizeBytes: 0 },
      { label: 'medium', width: 320, height: 320, key: `thumbnails/${attachmentId}/medium.webp`, mimeType: 'image/webp', sizeBytes: 0 },
      { label: 'large', width: 640, height: 640, key: `thumbnails/${attachmentId}/large.webp`, mimeType: 'image/webp', sizeBytes: 0 },
    ];
  }

  private async transcodeVideo(attachmentId: string, storageKey: string, mimeType: string) {
    // Placeholder - would use ffmpeg in production
    return [
      { resolution: '480p', codec: 'h264', bitrate: 1000, storageKey: `transcodes/${attachmentId}/480p.mp4`, sizeBytes: 0, duration: 0 },
      { resolution: '720p', codec: 'h264', bitrate: 2500, storageKey: `transcodes/${attachmentId}/720p.mp4`, sizeBytes: 0, duration: 0 },
      { resolution: '1080p', codec: 'h264', bitrate: 5000, storageKey: `transcodes/${attachmentId}/1080p.mp4`, sizeBytes: 0, duration: 0 },
    ];
  }
  ...
  private async extractMetadata(attachmentId: string, storageKey: string, mimeType: string) {
    return {
      mimeType,
  ...
  private async updateJobProgress(job: any, progress: number, status?: 'PENDING' | 'PROCESSING' | 'COMPLETED' | 'FAILED' | 'CANCELLED') {
