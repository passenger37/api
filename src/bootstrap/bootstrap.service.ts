...
export class BootstrapService implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap() {
    const enabled = this.config.get<boolean>('AUTH_BOOTSTRAP_ENABLED', false);

    if (!enabled) {
      this.logger.log('Authorization bootstrap disabled.');
      return;
    }

    this.logger.log('Running authorization bootstrap...');
