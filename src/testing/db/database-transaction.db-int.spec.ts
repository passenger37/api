...
describe('Database Integration: transactions (40.87)', () => {
  ...
  it('rolls back all writes when the transaction throws', async () => {
    const seed = await createChannelMessage(prisma, fixture, fixture.ownerMemberId, 'before', 3);
    ...
    const after = await prisma.channelMessage.findMany({
      where: { channelId: fixture.channelId, content: { in: ['should-rollback', 'breaks'] } },
    });
    expect(after).toHaveLength(0);

    // the write from a completed transaction is still there
    const preserved = await prisma.channelMessage.findUnique({ where: { id: seed.id } });
